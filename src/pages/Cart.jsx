...
import { PizzaCartBlock, EmptyCart } from '../components'
import { clearCart, removeCartItem, counterForItemPizz } from '../redux/actions/cart';
...
function Cart() {
  ...
  const pizzas = Object.keys(itemsCart).map(key => {
    return itemsCart[key].itemsCart[0]
  });
  ...
  const onRemoveItem = (objInf) => {
    if(window.confirm("Хотите удалить пиццу?"))
      dispatch(removeCartItem(objInf))
  }
  const onUpdateCounter = (infCount) => {
  
      dispatch(counterForItemPizz(infCount))
  }
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
          ...
          <span onClick={clearCRT}>Очистить корзину</span>
        </div>
      </div>
      <div className="content__items">

      {pizzas.map((obj, index) => (
   <PizzaCartBlock key={obj.id} 
   addedCart={obj} 
   totalPricePizza = {itemsCart[obj.id].totalPrice} 
   totalCountPizza = {itemsCart[obj.id].itemsCart.length}
   onRemove = {onRemoveItem}
   onUpdateCount = {onUpdateCounter}
   />
